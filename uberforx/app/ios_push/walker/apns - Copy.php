...
define("DefW", $cert_def);
$certi_user_a = Certificates::where('client','apple')->where('user_type',1)->where('type',$cert_def)->where('file_type','certificate')->first();
define("Certi_Path", $certi_user_a->name);
$pass_user_a = Certificates::where('client','apple')->where('user_type',1)->where('type',$cert_def)->where('file_type','passphrase')->first();
define("PassPH", $pass_user_a->name);

class Apns_old {

    private $passphrase = PassPH;
    private $certificateW = Certi_Path;
    private $defw = DefW;
    ...
    private function getCertificatePath() {
        Log::info("path =lker  = " . print_r(public_path().'/apps/ios_push/walker/iph_cert/'.$this->certificateW, true));
        return public_path().'/apps/ios_push/walker/iph_cert/'.$this->certificateW;
    }
